...
globals [ delta
          ...
          totalsize
          filenaam
          agentsorderedatstart
          triangles ; for partitioning in groups
          strategy-shapes
          plottitle
        ]
...
to go
   ...
   ask patches [  ; to forget
       set pprops map [ list (forget-pevidence first ?) (forget-pimportance second ?)] pprops ]
...
to act
  ...
  foreach [0 1 2] [
    ifelse item ? strategy-shapes = shape [set profit-strategy replace-item ? profit-strategy (size - prior-size) ]
      [set profit-strategy replace-item ? profit-strategy (item ? profit-strategy + 0.001) ]
  ]
end
...
to reply-attacks
if size > 1 [
   let pr  filter [[size] of first ? > 1] attacks ; only attacks one ofthe agents who have sufficient reputation
   if not empty? pr [
...
to setopinion [p evi] ; prop evidence importance
  ...
  ifelse length testconvex (replace-item p (map [first ?] props)  first evi) <= 1 ; new opion is convex 
...
; number of props with evidence > 0.8 (significance level)
  report length filter [first ? > 0.8] props
end

to-report LON
; number of props with evidence > 0.8 (significance level)
  report length filter [first ? <= 0.8 and first ? >= 0.2] props
end
...
to setup-plotfile
  ...
  file-open filenaam
  set agentsorderedatstart sort-by [[first item current-prop props] of ?1 < 
...
to-report clustering1
  createtriangles
  jointriangles
  report mean map [length ?] triangles / 20
end  
